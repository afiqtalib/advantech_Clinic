...
if (isset($_POST['submit'])) {
  ...
  $selectedValue = explode(',', $docname); // divide the input

  $value1 = $selectedValue[0]; // doc id
  $value2 = $selectedValue[1]; // doc first name
  $value3 = $selectedValue[2]; // doc last name

  $doctorname = "Dr $value2 $value3";

  $query = "SELECT COUNT(scheduleId) AS total FROM doctorschedule WHERE scheduleDate = '$date' AND startTime = '$starttime' AND endTime = '$endtime' AND doctorname = '$doctorname'"; //Count data for schedule

  $getDate = date("Y-m-d"); //Create current date

  $cnt = mysqli_query($con, $query);

  $data=mysqli_fetch_assoc($cnt);

  $data = $data['total']; // Total rows of data
/*
  $count_row = mysqli_num_rows($cnt);*/

  if($date > $getDate){
    if($data == 0){
      //INSERT
      $query = " INSERT INTO doctorschedule (scheduleDate, scheduleDay, startTime, endTime, bookAvail, doctorname, dr_id, icstaff)
      VALUES ( '$date', '$scheduleday', '$starttime', '$endtime', '$bookavail','$doctorname', '$value1', '$staffic') ";
      ...
      <?php
        ...
        ?>
        <script type="text/javascript">alert('Added fail. Please try again.');</script>
      <?php
      }
    }
    else{
      ?>
      <script type="text/javascript">alert("Data existed. Please resubmit the detail.");</script>
      <?php
    }
  }else{
    ?>
      <script type="text/javascript">alert("Date invalid. Please choose future date.");</script>
      <?php
  }
}
?>
        ...
        <div id="wrapper">
            ...
            <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
                ...
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="staffdashboard.php">ORAAS - ONLINE REGISTRATION AND APPOINTMENT SYSTEM</a>
                </div>
                <!-- Top Menu Items -->
                               ...
                               <div class="col-md-12 col-sm-12 col-xs-12">
                                <form class="form-horizontal" method="post">
                                 ...
                                 <div class="form-group form-group-lg">
                                  ...
                                  <div class="col-sm-10">
                                   <select class="select form-control" id="doctorname" name="doctorname" required>
                                    <option value="">----Select doctor----</option>
                                    <?php
                                    while($rows = mysqli_fetch_array($result1)):;?>\<option value="<?php echo $rows[2];?>, <?php echo $rows[3];?>,<?php echo $rows[4];?>">Dr <?php echo $rows[3];?> <?php echo $rows[4];?></option>
                                  <?php endwhile;?>
                                   </select>
                                  </div>
                                 </div>
                        ...
                        <table class="table table-hover table-bordered">
                            <thead>
                                <tr class="filters">
                                    ...
                                    <th><input type="text" class="form-control" placeholder="doctorname" disabled></th>
