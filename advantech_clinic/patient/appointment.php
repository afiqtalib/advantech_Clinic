...
if (isset($_POST['appointment'])){
	$patientIc = mysqli_real_escape_string($con,$_POST['icpatient']);
	$scheduleid = mysqli_real_escape_string($con,$_POST['appid']);
	$avail = "Booked";
	$status = "process";

	//$query = "INSERT INTO appointment (  patientIc , scheduleId , appSymptom , appComment  ) VALUES ( '$patientIc', '$scheduleid', '$symptom', '$comment') ";

	$find_patient = mysqli_query($con,"SELECT * FROM patient WHERE icPatient = '$patientIc'");
	$select_patient = mysqli_fetch_assoc($find_patient);

	if(empty($select_patient['patientFirstName']) OR empty($select_patient['patientLastName']) OR empty($select_patient['patientMaritialStatus']) OR empty($select_patient['patientDOB']) OR empty($select_patient['patientGender']) OR empty($select_patient['patientAddress']) OR empty($select_patient['patientPhone']) OR empty($select_patient['patientPhone'])){?>

		<script type="text/javascript">alert('Please update your profile first !');</script>
		<?php
	}else{
		$query = "INSERT INTO appointment (  patientIc , scheduleId , status  ) VALUES ( '$patientIc', '$scheduleid', '$status') ";
		...
		// echo $result;
		if( $result ){?>
			<script type="text/javascript">alert('Appointment made successfully.');</script>
		<?php
		...
		?>
			<script type="text/javascript">alert('Appointment booking fail. Please try again.');</script>
		<?php
		...
		<nav class="navbar navbar-default " role="navigation">
			<div class="container-fluid">
				...
				<div class="navbar-header">
					...
					<a class="navbar-brand" href="patient.php"><img alt="Brand" src="assets/img/logohalf.jpg" height="47px"></a>
				</div>
					...
					<div class="row">
						<div class="col-md-3 col-sm-3">
							...
							<div class="user-wrapper">
								<img src="assets/img/icon.png" class="img-responsive" />
								<div class="description">
									<h4><?php echo $userRow['patientFirstName']; ?> <?php echo $userRow['patientLastName']; ?></h4>
									
									<button type="button" class="btn btn-primary" onClick="myFunction()">Update Profile</button>
									<script>function myFunction() {
										window.location.href="profile.php?patientId=<?php echo $userRow['icPatient'];?>";
									}</script>
								</div>
							</div>
						</div>
										...
										<form class="form" role="form" method="POST" accept-charset="UTF-8">
											<div class="panel panel-default">
												...
												<div class="panel-body">
													...
													Patient Name: <input type="text" name="fname" value="<?php echo $userRow['patientFirstName'] ?>" readonly /> <input type="text" name="lname" value="<?php echo $userRow['patientLastName'] ?>" /readonly><br>
													Patient IC: <input type="text" name="icpatient" value="<?php echo $userRow['icPatient'] ?>" readonly /><br>
													Contact Number: <input type="text" name="contactno" value="<?php echo $userRow['patientPhone'] ?>" readonly /><br>
													Address: <input type="text" name="address" value="<?php echo $userRow['patientAddress'] ?>" readonly />
												</div>
											</div>
											<div class="panel panel-default">
												<div class="panel-heading">Appointment Information</div>
												<div class="panel-body">
													Day: <?php echo $userRow['scheduleDay'] ?><input type="hidden" name="appid" value="<?php echo $appid ?>" /><br>
													Date: <?php echo $userRow['scheduleDate'] ?><input type="hidden" name="scheduledate" value="<?php echo $appdate ?>" /><br>
